...
const Hyperspeed = ({
  ...
  useEffect(() => {
    const root = hyperspeed.current;
    // We'll crossfade, so don't nuke existing layer here
    ...
    (function () {
      const rootEl = hyperspeed.current;
      if (!rootEl) return;
      // create new layer to crossfade in
      const container = document.createElement('div');
      container.className = 'hs-layer';
      ...
      myApp.loadAssets().then(() => {
        if (!myApp || myApp.disposed) return;
        myApp.init();
        // crossfade
        requestAnimationFrame(() => {
          container.style.opacity = '1';
          container.style.pointerEvents = 'auto';
          if (oldLayer) oldLayer.style.pointerEvents = 'none';
        });
        // cleanup old after transition
        setTimeout(() => {
          try { if (oldApp) oldApp.dispose(); } catch {}
          try { if (oldLayer && oldLayer.parentNode) oldLayer.parentNode.removeChild(oldLayer); } catch {}
        }, 1100);
      });
    })();
    ...
    return () => {
      ...
      try { if (app) app.dispose(); } catch {}
      try {
        if (hyperspeed.current && layer && layer.parentNode === hyperspeed.current) {
          hyperspeed.current.removeChild(layer);
        }
      } catch {}
      appRef.current = null;
